...
# results directory
RES_DIR = 'results/bhv_{}_perm'.format(model_type)
...
def _test(df, bhv_df, args):
    ...
    for score in SCORES:
        ...
        results['test_%s'%score] = np.zeros(num_perms)
        results['y'] = np.zeros((num_perms, 18*len(test_list), 260))
        results['y_hat'] = np.zeros((num_perms, 18*len(test_list), 260))
        results['c'] = np.zeros((num_perms, 18*len(test_list), 260))

        for ii in range(k_clip):
            results['t_train_%s'%score][ii] = np.zeros(clip_time[ii])
            results['t_test_%s'%score][ii] = np.zeros((num_perms, clip_time[ii]))
    ...
    '''
    for perm in tqdm(range(num_perms)):
        X_test, test_len, y_test, c_test = _get_seq(
            df, test_list, args, label_shuffle = True)
        s, s_t, y, y_hat, c = dnn_score(
            model, X_test, y_test,
            c_test, test_len, max_length, 
            clip_time, model_type = model_type)
        for score in SCORES:
            results['test_%s'%score][perm] = s[score]
            for ii in range(k_clip):
#                 print(s_t[ii][score].shape, results['t_test_%s'%score][ii].shape, results['t_test_%s'%score][ii][perm,:].shape)
                results['t_test_%s'%score][ii][perm,:] = s_t[ii][score]
#         print('test p = %0.3f' %s['p'])
#         print(y.shape, y_hat.shape, c.shape)
        results['y'][perm] = y
        results['y_hat'][perm] = y_hat
        results['c'][perm] = c
