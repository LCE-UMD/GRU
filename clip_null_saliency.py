            ...
            for subject in subject_list:
                ...
                if i_class==0: # must handle test retest differently
                    ...
                    if clip_time[i_class] > clip_time[class_label]:
                        gradX = np.vstack([gradX, np.zeros((clip_time[i_class] - clip_time[class_label], gradX.shape[1]))])

                df.loc[(df['Subject']==subject) & 
                    (df['y'] == i_class), grads] = gradX[0:clip_time[i_class], :]

        sal_df = df[['Subject', 'timepoint', 'y'] + grads]
        
        with open("/home/joyneelm/movie-clip-predictions/results/null_saliency/label_shuffle_starwars/perm_{:03}.pkl".format(idx_perm), 'wb') as f:
